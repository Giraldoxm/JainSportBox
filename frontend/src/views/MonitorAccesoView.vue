<template>
  <div class="animate-fade-in-up">
    <div class="mb-6">
      ...
      <p class="text-gray-500 mt-1">Busca un cliente manualmente o usa el lector de huella para acceso automático</p>
    </div>
    ...
    <div class="mb-6 rounded-2xl border p-5 flex flex-col sm:flex-row items-start sm:items-center gap-5 transition-all"
      ...
      <div class="relative flex-shrink-0">
        <div v-if="bridgeConectado && !ultimoEvento" class="absolute inset-0 rounded-full bg-red-400 animate-ping opacity-30"></div>
        <div class="relative w-16 h-16 rounded-full flex items-center justify-center"
          :class="bridgeConectado ? 'bg-red-100 border-2 border-red-300' : 'bg-gray-100 border-2 border-gray-200'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" :class="bridgeConectado ? 'text-red-600' : 'text-gray-400'" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <span class="w-2 h-2 rounded-full flex-shrink-0"
            :class="bridgeConectado ? 'bg-red-500' : 'bg-gray-300'"></span>
          <p class="font-bold" :class="bridgeConectado ? 'text-red-800' : 'text-gray-500'">
        ...
        <p v-else-if="bridgeStatus?.modo === 'enrolando'" class="text-sm text-red-600 font-medium">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="col-span-1 lg:col-span-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-[600px]">
        ...
        <ul v-else class="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
          ...
          >
            <img class="h-10 w-10 rounded-full object-cover bg-gray-200" :src="fotoSrc(user)" alt="" />
            <div class="flex-1 min-w-0">
              <p class="text-sm font-bold text-gray-900 truncate">{{ user.nombre }}</p>
              <p class="text-xs text-gray-500 truncate">ID: {{ user.id }}</p>
            </div>
            <div>
              <span v-if="isVigenteCheck(user)" class="w-3 h-3 bg-emerald-500 rounded-full inline-block shadow-sm"></span>
      ...
      <div class="col-span-1 lg:col-span-2">
        <div v-if="!selectedUser" class="h-full bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-12 text-center text-gray-400 transition-all">
          ...
          <p class="text-xl font-semibold">Selecciona un cliente a la izquierda</p>
        ...
        <div v-else class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden transform transition-all relative">
          ...
          <div :class="isVigente ? 'bg-emerald-500' : 'bg-red-500'" class="h-32 absolute top-0 left-0 w-full transition-colors z-0"></div>
          
          <div class="relative z-10 px-8 pt-20 pb-8 flex flex-col items-center">
            <img class="h-32 w-32 rounded-full border-4 border-white shadow-xl bg-white object-cover" :src="fotoSrc(selectedUser)" alt="" />
            ...
            <div class="mt-6 w-full">
              <div 
                :class="isVigente ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'"
              ...
              >
                ...
                <p class="text-4xl font-black tracking-tight" :class="isVigente ? 'text-emerald-600' : 'text-red-600'">
